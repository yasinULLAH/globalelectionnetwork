...
import { CANDIDATES, PARTIES, CONSTITUENCIES } from '@/lib/mockData';
  ...
  try {
    ...
    if (!candidates.length) {
      candidates = CANDIDATES.map(c => {
        const p = PARTIES.find(party => party.id === c.partyId);
        const con = CONSTITUENCIES.find(constituency => constituency.id === c.constituencyId);
        return {
          ...c,
          party_id: c.partyId,
